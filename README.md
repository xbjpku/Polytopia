...
### /polytopia
Source code for the game. It currently consists of 3 packages:
...
* `Unit.java`: Describes a unit in the game. Placeholder for now.
...
Implements graphics for the game. Incomplete for now.
...
```
For example, to run the demo in `Tile.java`, type in
```
java -cp bin polytopia.gameplay.Tile 
```
